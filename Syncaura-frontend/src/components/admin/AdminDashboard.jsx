...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      .then(([projectsRes, tasksRes, notifRes, usersRes]) => {
        ...
        if (notifRes.status === "fulfilled" && notifRes.value?.data) {
          ...
          setNotifications(data.data || (Array.isArray(data) ? data : []));
        }
        if (usersRes.status === "fulfilled" && usersRes.value?.data) {
          const data = usersRes.value.data;
          setTotalUsers(Array.isArray(data) ? data.length : Array.isArray(data.users) ? data.users.length : 0);
        }
      })
      .catch((err) => {
        console.warn("Failed loading some dashboard metrics:", err);
      })
    ...
    <div className="overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 gap-6">
        <motion.div variants={item} initial="hidden" animate="show">
          <ProjectStatus projects={projects} tasks={tasks} loading={loading} />
        </motion.div>
        <motion.div variants={item} initial="hidden" animate="show">
          <ProjectRisks projects={projects} tasks={tasks} loading={loading} />
        </motion.div>
        <motion.div variants={item} initial="hidden" animate="show">
          <ProductivityTrend />
        </motion.div>
        ...
        <motion.div variants={item} initial="hidden" animate="show">
          <IssuesAndAlerts notifications={notifications} loading={loading} />
        </motion.div>
      </div>
    </div>
  );
