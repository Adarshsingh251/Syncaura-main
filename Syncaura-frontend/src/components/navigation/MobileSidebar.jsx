...
export default function MobileSidebar({ open, setOpen }) {
  ...
  const menuItems = [
    { label: "dashboard", icon: LayoutDashboard, path: dashboardPath, count: 0 },
    ...
    { label: "attendance", icon: Clock, path: "/attendance-leave", count: 0 },
    { label: "myAttendance", icon: UserCheck, path: "/my-attendance", count: 0 },
    { label: "Issue Status", icon: ClipboardCheck, path: "/issue-status", count: 0 },
  ...
  const totalUnreadChats =
    channels?.reduce((acc, chat) => acc + (parseInt(chat.unread) || 0), 0) || 0;

  // Dynamically update the Chat menu item's count
  const dynamicMenuItems = menuItems.map((item) =>
    item.label === "chat" || item.label === "Chat"
      ? { ...item, count: totalUnreadChats }
      : item
  );
